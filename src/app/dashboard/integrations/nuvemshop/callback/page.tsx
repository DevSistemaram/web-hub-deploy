...
function NuvemshopCallback() {
  ...
  useEffect(() => {
    ...
    api.integrations.handleNuvemshopCallback(code)
      .then(() => {
        setStatus('success');
        setTimeout(() => router.push('/dashboard/integrations'), 2000);
      })
      .catch((err) => {
        setErrorMessage(err instanceof Error ? err.message : 'Falha ao vincular Nuvemshop.');
        setStatus('error');
      });
  }, [router, searchParams]);
        ...
        <CardContent className="pt-10 pb-10 text-center">
          {status === 'loading' && (
            <>
              ...
              <h2 className="text-xl font-semibold text-foreground mb-2">Vinculando Nuvemshop...</h2>
          ...
          {status === 'success' && (
            <>
              ...
              <h2 className="text-xl font-semibold text-foreground mb-2">Nuvemshop vinculada!</h2>
...
export default function NuvemshopCallbackPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-background">
